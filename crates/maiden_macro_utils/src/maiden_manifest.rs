...
pub struct MaidenManifest {
    manifest: DocumentMut,
}

impl Default for MaidenManifest {
    fn default() -> Self {
        Self {
...
const MAIDEN: &str = "maiden";
const MAIDEN_INTERNAL: &str = "maiden_internal";
...
impl MaidenManifest {
    pub fn maybe_get_path(&self, name: &str) -> Option<syn::Path> {
        ...
        let find_in_deps = |deps: &Item| -> Option<syn::Path> {
            ...
            } else if let Some(dep) = deps.get(MAIDEN) {
                println!("MAIDEN 의존성 확인: {:?}", dep);
                dep_package(dep).unwrap_or(MAIDEN)
            } else if let Some(dep) = deps.get(MAIDEN_INTERNAL) {
                println!("MAIDEN INTERNAL 의존성 확인: {:?}", dep);
                dep_package(dep).unwrap_or(MAIDEN_INTERNAL)
            } else {
                return None;
            };

            let mut path = Self::parse_str::<syn::Path>(package);
            if let Some(module) = name.strip_prefix("maiden_") {
